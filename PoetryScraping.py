...
def pageOne(letter = 'a'):
    ...
    for j in linksAdd:
        ...
        with open('Data/CSV_Site1.csv', 'a', errors="ignore") as csvfile:
            ...
            tempDict['location'] = exportJson['location']
            tempDict['poem'] = exportJson['poem']
...
def pageTwo(letter = 'a'):
    ...
    for link in linksTwo:
        ...
        if (pre_allLines != None):
            ...
            allLines = newSoupTwo.find("div", {"class" : "content-inner"}).find_all('p')
            tempString = ""
            for line in allLines:
                tempString = tempString + line.text
            poemsTwo.append(tempString)
    print("Stopper")
    for  bio in bioBuild:
...
def pageThree(number):
    ...
    print("----------------------------------------------------------------------")
    with open('CSV_Site3.csv', 'a',errors="ignore") as csvfile:
...
def main():
    ...
    for char in alphabet:
        pageTwo(char)
    #     Note on pages 1001 - end are not worth scraping as they are images from books!
    for x in range(0, 2276):
        print(x)
        pageThree(x)
